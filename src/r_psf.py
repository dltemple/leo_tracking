...
def inject(image, n_stars, PSF_mod, grid=0, out_name='inject.fits', flux_dist=9000, chop_x=3, chop_y=3, overwrite=False, write_cat=True, x_p=-1, y_p=-1):
    ...
    if PSF_mod == 0:
        image = inject_fake_stars(image, n_stars, grid, flux_dist, PSF_mod, write_cat, x_p, y_p, chop_x, chop_y)
    else:
        image = inject_fake_stars(image, n_stars, grid, flux_dist, PSF_mod, write_cat, x_p, y_p)
