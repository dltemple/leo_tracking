...
if __name__ == '__main__':
    ...
    median_arr = cv2.resize(median_arr, (6000, 4000), interpolation=cv2.INTER_AREA)

    prev_arr = None

    observer.median_image = median_arr

    # out_array = np.zeros([6000 * 4000, len(images[1:])])
    ...
    for idx, image_file in enumerate(images[:]):
        ...
        if write_ofp:
            print(idx)
            out_array = bkg_subtracted_image[minx:maxx, miny:maxy].reshape([-1])
        ...
        # with open('test.ofp', 'wb') as f:
        #     corrected_image.astype('d').tofile(f)
        #     np.array(4000).astype('int').tofile(f)
        #     np.array(6000).astype('int').tofile(f)
        #     np.array(1).astype('int').tofile(f)
        ...
        # if idx > 0:
            ...
            # corrected_image = cv2.Canny(corrected_image, 100, 200)
            # corrected_image = auto_canny(corrected_image)
            # resized = cv2.resize(corrected_image, (600, 400), interpolation=cv2.INTER_AREA)
    ...
    # np.array(512).astype('int').tofile(f)
    # np.array(idx).astype('int').tofile(f)
