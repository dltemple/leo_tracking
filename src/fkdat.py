...
def do_inject(im, X, Y, p_mod, flux):
    ...
    # print(np.sum(kernel))
    # print(kernel)
    return im, kernel * flux


def make_noise(image):
    return np.random.normal(3, 0.35, size=image.shape)
...
def fake_data(X_size, Y_size, pix_size=None, num_sources=500, pos=None, fluxes=None, noise=None, PSF=None, outname='out.fits', overwrite=False):
    ...
    for i in range(len(pos)):
        image, kernel = do_inject(image, pos[i][0], pos[i][1], PSF, fluxes[i])
        kernel_sums.append(np.sum(kernel))

    return image, np.array(kernel_sums)
